    ...
    return (
        ...
        >
            <div style={{height: '260px', cursor: 'pointer', position: 'relative'}} onClick={onViewDetails} className="product-image-container">
                <ImageLoader src={imageSrc} alt={cake.name} className="w-100 h-100" />
                    ...
                    <div className="mb-4">
                        ...
                        <div>
                            {cake.flavors && cake.flavors.map((flav, i) => (
                                <span key={i} className="badge bg-light text-dark border me-1 mb-1 fw-normal" style={{fontSize: '0.75rem'}}>{flav}</span>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="mt-auto">
                    <button onClick={onOrder} className="btn btn-outline-danger w-100 rounded-pill fw-bold">Order Now</button>
                </div>
            </div>
            <style jsx>{`
